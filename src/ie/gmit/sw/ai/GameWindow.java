...
public class GameWindow extends Application{
    ...
    public void keyPressed(KeyEvent e) { //Handle key events
        ...
        if (key == KeyCode.RIGHT && currentCol < DEFAULT_SIZE - 1) {
        	if (model.isValidMove(currentRow, currentCol, currentRow, currentCol + 1, PLAYER_ID, null)) currentCol++;   		
        }else if (key == KeyCode.LEFT && currentCol > 0) {
        	if (model.isValidMove(currentRow, currentCol, currentRow, currentCol - 1, PLAYER_ID, null)) currentCol--;	
        }else if (key == KeyCode.UP && currentRow > 0) {
        	if (model.isValidMove(currentRow, currentCol, currentRow - 1, currentCol, PLAYER_ID, null)) currentRow--;
	...
	private void placePlayer(){  //Place the main player character	
    	...
    	model.set(currentRow, currentCol, PLAYER_ID, null); //Player is at index 1
    	updateView(); 		
	}
